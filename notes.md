...
create django project -> create django app -> create view within the app -> create models within the app ->

## -------------------------

## littlelemon project

this is going to build a project called littlelemon and an app called restaurant

## steps

1. install django in the project directory ==> pipenv install django
2. activate the virtual environment if not already activated ==> pipenv shell
3. create your model in the model.py file
4. next you update admin.py and import the models you just created above
5. next step will be to perform the migrations
   - open terminal
   - navigate to littlelemon directory (project directory)
   - run the migration commands: python manage.py makemigrations and python manage.py migrate
6. after migration is done, next is to do a url mapping both for the project level and app level
   - to do a project level url mappings, go to the project and open its urls.py file and add the path of the app level url
   - to do an app level url mappings, go to the app directory and open its urls.py file and add the corresponding paths
7. next will be to create the menu items using the admin panel
   - to do this, we need access to the admin panel and that will mean the superuser has to be created
   - open the terminal and run "python manage.py createsuperuser" to create the superuser
8. once the superuser is created, you can now run the application and access the admin panel
   - to run the application, use the command: python manage.py runserver
9. next will be to update the views file to create the homepage or the page accessible for end users to create/update their records
10. add the path of the view created above to the app level url
11. next will be to look at the template, which has the html files
